...
class Base3d {
  ...
  render() {
    var delta = this.clock.getDelta();    //拿到帧与帧之间的时间差
    this.mixer && this.mixer.update(delta);  //拿到时间差之后让混合器更新物体的数据
    this.renderer.render(this.scene, this.camera);  // 更新之后物体随之变化然后通过渲染器渲染出来
  }
  animate() {
